...
| **R-11** | Better Auth 導入までの間、`lib/auth/session.ts` は Cookie の値をそのまま `userId` として信頼する暫定セッションであり、実際の認証を行わない（`/security-review` 指摘、M2 UI着手時に導入）。誰でも `setDevUser` を呼べば任意の開発用ユーザーになりすませる | **高（公開時のみ）** | ローカル開発・Vercel Preview を **一般公開しない**（Vercel の Deployment Protection 等でアクセス制限する）限りは実害がない。Better Auth 導入（M1 #10）までは、この暫定セッションのまま**本番URLを一般公開しない**こと。導入後は `lib/auth/session.ts` の実装のみを差し替える（呼び出し側のシグネチャは変えない） |
