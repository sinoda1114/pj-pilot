...
> **⚠️ この節だけは「作成時点（2026-08-06）のスナップショット」ではなく、最新の状態に更新しています。**
> §2 以降は作成時点の計画・調査結果をそのまま残しています（決定の根拠を追えるようにするため）。

### 1.1 現在の状態（2026-08-08 時点。実物で確認）

**Phase 1（M0〜M6）と Phase 2（M7〜M9）のコードはすべて実装・マージ済み**です。
残っているのは、クラウドセッションから到達できない外部サービス側の設定だけです。

| 項目 | 状態 |
|---|---|
| Next.js アプリ（Phase 1: 複数PJ / 課題管理 / WBS / Gantt / 依存連動） | ✅ 実装済み |
| Phase 2（カンバンボード / ダッシュボード / CSV エクスポート） | ✅ 実装済み |
| Better Auth（Google OAuth 限定・ドメイン制限）| ✅ 実装済み（R-11 解消） |
| `.github/`（CI ワークフロー・dependabot） | ✅ あり |
| テスト | ✅ ユニット 577 / E2E 34 |
...
以下は本ドキュメント作成時点の記録です。§2 以降の判断の前提になっているため残します。

リポジトリは「設計フェーズの器」だけがある状態でした。アプリケーションコードは 1 行もありません。
...
>
> （その後 `origin/HEAD` は設定済みになりました。`type:*` ラベルは 2026-08-08 時点でも未作成です。）
...
> **更新: 2026-08-08。** 作成時点の内容（データモデルの承認待ち・M0 着手）はすべて完了したため、
> 現在の残作業に置き換えています。実施手順の詳細は [`docs/LOCAL_SETUP.md`](LOCAL_SETUP.md)。
...
作成時点の割り振り（クラウドセッションは伝播ロジック、ローカルはブラウザ確認が要る部分）は
役目を終えました。現状はコードがクラウドセッション側で完結しており、**残タスクはすべてローカル**です。

クラウドセッションの egress ポリシーが vercel.com / turso.tech / Google Cloud Console への
通信を遮断していることは実測で確認済みです（プロキシから 403）。
