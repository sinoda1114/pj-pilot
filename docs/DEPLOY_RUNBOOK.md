...
なお、このワークフローは `GITHUB_TOKEN` で ref を更新します。Bot による更新で Vercel のデプロイが発火するかは事前には確定できませんでしたが、PR #73 のマージ時に実測して**発火することを確認済み**です。ref 更新の6秒後に Preview デプロイが新規作成され、固定ドメインの向き先もそれに切り替わりました。`GITHUB_TOKEN` の push が再帰実行を防ぐために抑制されるのは Actions のワークフロー起動に限った仕様で、GitHub App（Vercel）への webhook 配信は別扱いです。**Deploy Hook を挟む必要はありません。**
