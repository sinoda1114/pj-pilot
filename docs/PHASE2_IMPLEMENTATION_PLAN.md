...
> - **未決事項の扱い**: Q-1・Q-2 は既定のまま（進捗の自動変更なし / 期限超過は20件超で
>   「もっと見る」）。**Q-3（カンバンのフィルター）と決定 P2-12（CSV エクスポート）は
>   後から方針が変わり、いずれも追加実装した**（§9・§11 参照）。
> - **カバレッジ**: 補強により Stmts 95.67 / Branch 92.88 / Funcs 94.14 / Lines 95.46 まで
>   上がったため、`vitest.config.ts` の閾値を 85/75/85/85 から 94/90/92/94 へ引き上げた。
...
未決事項（計画時点。**結論は下記のとおり確定済み**）:

- **Q-1**: 完了列へのドロップで `progress` を 100 にするか → **確定: しない**（§5.7）
- **Q-2**: ダッシュボードの期限超過一覧の表示件数上限 → **確定: 20件超で「もっと見る」**
- **Q-3**: カンバンにフィルター（担当者・優先度）を付けるか → **⚠️ 既定から変更。付けました。**
  計画時点では「ヒアリングの表示項目に無くスコープを膨らませない」として見送る既定でしたが、
  Phase 2 完了後に追加実装しました（`lib/board/filter.ts`）。優先度と担当者の複数選択で、
  条件間は AND・条件内は OR、「担当者なし」も選べます。
  **絞り込み中も D&D の `toIndex` は「その列の全タスクの中での位置」を使います**
  （描画用の配列と位置計算用の配列を別に持つ）。見た目の位置を使うと、
  隠れているカードの分だけ index がずれて意図しない場所へ飛びます。
...
| P2-12 | CSV エクスポートは含めない → **⚠️ 撤回。実装しました** | 計画時点ではヒアリングで対象外としたが、Phase 2 完了後に追加実装した（`lib/export/csv.ts`）。RFC 4180 準拠のエスケープ、Excel 向けの UTF-8 BOM は付与を分離。タスク一覧は**画面で絞り込んだ後の行**、ダッシュボードは期限超過の**全件**を出す |
