...
- **孤児の回収**: SIGKILL などで EXIT の trap が走らないと worktree が残る。ディレクトリ名に作成したプロセスの PID を入れ（`ai-review-wt.<pid>.XXXXXX`）、次の起動時にそのプロセスがいなければ回収する
...
- **検証**: `scripts/test-isolation.sh`（`AI_REVIEW_DRY_RUN=1` で実レビュアーは起動しない）。34 項目。レビュアーの記録は終了時点の状態を取るので、レビュー中の書き換えも捉えられる。対照群で次を確かめた。隔離を外すと「レビュー中の書き換えが混ざらない」が失敗する。TERM の処理を外すと「レビュアーも止まる」が失敗する。SHA の照合を外すと「HEAD が動いたら記録を拒む」が失敗する
